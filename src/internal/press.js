// internal/press.js
//
// Same contract as press-legacy.js (press state plus the native touch/tap
// wiring every interactive component shares), one real difference: no
// explicit redraw call at all. The legacy mithril-lynx deliberately
// normalizes every event with `redraw: false` (a redraw on every touch
// event would be wasteful there), so press-legacy.js has to ask for one
// itself. The current mithril-lynx's own commit.js has no such opt-out —
// Mithril's real `EventDict.handleEvent` auto-redraws after ANY event
// handler runs, unconditionally, which is exactly what makes
// `state.pressed = next` alone enough here.
